...
from recycle_databox import RecycleDataBox, RecycleDataView
...
<DataButton>:
    canvas.before:
        Color:
            rgb: 0.2, 0.3, 0.6, 1
        Rectangle:
            size: self.width, self.height * 0.96
...
class DataButton(RecycleDataView, Label):
...
class RecycleDataBoxExample(App):
    starttime = time()
    frametimes = []
    ...
    def test2(self, *args):
        if self.root.ids.data_box.data:
            self.root.ids.data_box.data = self.root.ids.data_box.data[
                :int(len(self.root.ids.data_box.data) / 2)]
